...
DTI_DS=zeros(size(DTI));


for k=1:size(DTI,3)
    for i=1:size(DTI,2)
        for j=1:size(DTI,1)   
           DTI_DS(j,i,k,:)= [DTI(i,j,k,1) DTI(i,j,k,4) DTI(i,j,k,6) DTI(i,j,k,2) DTI(i,j,k,3) DTI(i,j,k,5)];           
        end
    end
end

fname=sprintf('DTI_DS.d');
fid = fopen(fname,'w+','l');
fwrite(fid,DTI_DS,'float');
fclose(fid);   
